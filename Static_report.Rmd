...
which may change as time passes.  The most common case definitions are met by few patients in our dataset, which varies by age. 
...
The frequency of presenting symptoms varies with age (Figure 6). Altered consciousness increases with age. Fever, respiratory and constitutional symptoms are mostly present in patients aged 40 and above.  The presence of gastro-intestinal symptoms appears to be age-independent. 
...
The age distribution of the most frequent concomitant conditions is presented In Figure 8. Dementia and hypertension increase with age. 
...
### Vital signs
Vital signs are presented in Figure 8 by age. `r (nrow(summary_input %>% filter(is.na(oxygen_therapy)==F)))` (`r round(nrow(summary_input %>% filter(is.na(oxygen_therapy)==F))/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%) patients were admitted on an oxygen therapy and no information was present for `r (nrow(summary_input %>% filter(is.na(oxygen_therapy)==T)))` (`r round(nrow(summary_input %>% filter(is.na(oxygen_therapy)==T))/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%). The panel with oxygen saturation depicts O2 on room air for those patients that didn't receive O2 on admission (`r nrow(data_plot_vs_oxysat)` (`r round(nrow(data_plot_vs_oxysat)/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%)). `r paste0(nrow(summary_input %>% filter(vs_oxysat<94)),'/',nrow(summary_input %>% filter(is.na(vs_oxysat)==F)))` (`r round(nrow(summary_input %>% filter(vs_oxysat<94))/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%) patients presented with oxygen saturation <94%, `r paste0(nrow(data_plot_vs_oxysat %>% filter(value<94)),'/',nrow(data_plot_vs_oxysat))` (`r round(nrow(data_plot_vs_oxysat %>% filter(value<94))/nrow(data_plot_vs_oxysat)*100,1)`%) patients with oxygen saturation <94% on room air.
...
### Laboratory values 
Laboratory tests results are available for a minority of patients. They are summarised in Figure 10a (biochemistry) and 10b (haematology and C-reactive protein). Urea increases with age.
...
\newpage
The distribution of the number of days from admission to ICU admission is shown in Figure 14. 
...
\newpage
The observed mean duration for the number of days from hospital admission to outcome (death or discharge) was `r round(mean((summary_input%>%filter(slider_outcome=="Death"|slider_outcome=="Discharge"))$dur_ho, na.rm=T),1)`, with SD `r round(sd((summary_input%>%filter(slider_outcome=="Death"|slider_outcome=="Discharge"))$dur_ho, na.rm=T),1)` days and a median of `r median((summary_input%>%filter(slider_outcome=="Death"|slider_outcome=="Discharge"))$dur_ho, na.rm=T)` days. These estimates are based on all cases which have complete records on length of hospital stay (N = `r nrow(summary_input %>%filter((slider_outcome=="Death"|slider_outcome=="Discharge")&! is.na(dur_ho)))`).The distributions of length of hospital stay are presented In Figure 15a according to sex and 15b by patient age group.  


```{r  staybysex, out.width= "100%", fig.height=3, fig.cap="Distribution of length of hospital stay, according to sex (a) and patient age group (b). This only includes cases with reported outcomes. The coloured areas indicate the kernel probability density of the observed data and the box plots show the median and interquartile range of the variable of interest."}

length.of.stay.sex.plot(length.of.stay.sex.input) + ggtitle("(a)")
```

```{r  staybyage, out.width= "100%", fig.height=3}

length.of.stay.age.plot(length.of.stay.age.input) + ggtitle("(b)")
```
