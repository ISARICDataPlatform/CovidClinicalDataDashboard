...
One-and-half year and over 3,800,000 deaths on [1], COVID-19 is clearly showing us how pandemics disrupt populations by stretching healthcare systems, shaking economies, and killing millions. Although clinicians and well-equipped surveillance systems can adapt to health emergencies by implementing new treatments and information systems, clinical research lacks this agility.
...
```{r cohortoverview,  out.width = "180%", fig.align="center", fig.cap = "Overview of cohort and outcomes as of 25 May 2021."}
...
### Vital signs
Vital signs are presented in Figure 10 by age. `r nrow(summary_input %>% filter(oxygen_therapy)==T)` (`r round(nrow(summary_input %>% filter(oxygen_therapy==T))/nrow(summary_input %>% filter(is.na(oxygen_therapy)==F))*100,1)`%) patients with oxygen therapy recorded were admitted on an oxygen therapy and no information was present for `r nrow(summary_input %>% filter(is.na(oxygen_therapy)==T))` (`r round(nrow(summary_input %>% filter(is.na(oxygen_therapy)==T))/nrow(summary_input)*100,1)`%). The panel with oxygen saturation depicts O2 on room air for those patients that didn't receive O2 on admission (`r nrow(data_plot_vs_oxysat)` (`r round(nrow(data_plot_vs_oxysat)/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%)). `r paste0(nrow(summary_input %>% filter(vs_oxysat<94)),'/',nrow(summary_input %>% filter(is.na(vs_oxysat)==F)))` (`r round(nrow(summary_input %>% filter(vs_oxysat<94))/nrow(summary_input %>% filter(is.na(vs_oxysat)==F))*100,1)`%) patients presented with oxygen saturation <94%, `r paste0(nrow(data_plot_vs_oxysat %>% filter(value<94)),'/',nrow(data_plot_vs_oxysat))` (`r round(nrow(data_plot_vs_oxysat %>% filter(value<94))/nrow(data_plot_vs_oxysat)*100,1)`%) patients with oxygen saturation <94% on room air.
...
### Laboratory values 
Laboratory tests results are available for a minority of patients. They are summarised in Figure 11. Urea increases with age.
...
Partners can submit ad-hoc analyses to access the half-a-million case-based dataset using our Statistical Analysis Plan form (SAP; https://isaric.org/research/isaric-partner-analysis-frequently-asked-questions/). Once a SAP is submitted, our statistical team assesses the analysis plan for quality of research and feasibility of data. Then, our clinical team provides feedback on clinical relevance and overlapping with other analyses. Once this process is finalised, the SAP is sent to all ISARIC collaborators for review. This initial peer review process ensures that research is of utmost quality and a truly participatory approach. 
...
True international collaboration during a pandemic is key to putting data into action. ISARIC partners have achieved this objective, and the results are shown in this special issue with half-a-million patients. 
