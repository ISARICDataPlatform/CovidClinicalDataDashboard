...
The cohort satisfying the above criteria has **`r nrow(summary_input)`** cases. The flow chart (Figure 1) gives an overview of the cohort and outcomes as of June 2021. 

```{r cohortoverview,  out.width = "200%", fig.align="center", fig.cap = "Overview of cohort and outcomes as of June 2021."}

flowchart()

```

These data have been contributed to by `r length(unique(summary_input_overall$siteid_final))` sites in `r length(unique(summary_input_overall$slider_country))` countries (see Figure 2 and Figure 3). N (%) of the patients are from low- and middle-income and N (%) from high-income countries. The two largest contributors to the database are the UK (`r round(nrow(patient.by.country.input[patient.by.country.input$slider_country=="United Kingdom",])/nrow(patient.by.country.input)*100,1)`%) and South Africa (`r round(nrow(patient.by.country.input[patient.by.country.input$slider_country=="South Africa",])/nrow(patient.by.country.input)*100,1)`%). 
...
```{r table3, tab.cap = "Prevalence of comorbidities."}
...
```{r table6, tab.cap = "Outcome by age and sex. Proportions are calculated using the column total as the denominator."}
