...
diagram <- "digraph flowchart {
      ...
      [1]: paste0('All patients in ISARIC database \\n (N=', nrow(flowchart_input), ')')
      [2]: paste0('EXCLUDED X%')
      [3]: paste0('ANALYSED X%')
      [4]: paste0('<14-days follow-up\\n (N=XXX)')
      [5]: paste0('>14-days\\n follow-up and\\n negative or not\\n confirmed\\n (N=XXX)')
      [6]: paste0('>14-days\\n follow-up and\\n positive for COVID-19 \\n (N=', nrow(flowchart_input), ')')
      [7]: paste0('ICU/HDU \\n (N=', nrow(flowchart_input %>% filter(ICU==T)), ')')
      [8]: paste0('In hospital\\n (N=', nrow(flowchart_input %>% filter(ICU==T & group=='In hospital')),')')
      [9]: paste0('Deceased\\n (N=', nrow(flowchart_input %>% filter(ICU==T & group=='death')), ')')
      [10]: paste0('Discharged\\n alive (N=', nrow(flowchart_input %>% filter(ICU==T & group=='discharge')), ')')
      [11]: paste0('Lost to follow-up\\n (N=', nrow(flowchart_input %>% filter(ICU==T & group=='LFTU')), ')')
      [12]: paste0('No ICU/HDU or ICU/HDU\\nstatus unknown\\n (N=', nrow(flowchart_input %>% filter(ICU==F)),')')
      [13]: paste0('In hospital\\n (N=', nrow(flowchart_input %>% filter(ICU==F & group=='In hospital')),')')
      [14]: paste0('Deceased\\n (N=', nrow(flowchart_input %>% filter(ICU==F & group=='death')), ')')
      [15]: paste0('Discharged\\n alive (N=', nrow(flowchart_input %>% filter(ICU==F & group=='discharge')), ')')
      [16]: paste0('Lost to follow-up\\n (N=', nrow(flowchart_input %>% filter(ICU==F & group=='LFTU')), ')')
      [17]: 'X%'
      [18]: 'X%'
      [19]: paste0(round(prop.table(table(flowchart_input$ICU))[2]*100,0), '%')
      [20]: paste0(round(prop.table(table(flowchart_input$ICU))[1]*100,0), '%')
      [21]: paste0(round(prop.table(flowchart_input %>% filter(ICU==T) %$% table(group))[3]*100,0), '%')
      [22]: paste0(round(prop.table(flowchart_input %>% filter(ICU==T) %$% table(group))[1]*100,0), '%')
      [23]: paste0(round(prop.table(flowchart_input %>% filter(ICU==T) %$% table(group))[2]*100,0), '%')
      [24]: paste0(round(prop.table(flowchart_input %>% filter(ICU==T) %$% table(group))[4]*100,0), '%')
      [25]: paste0(round(prop.table(flowchart_input %>% filter(ICU==F) %$% table(group))[3]*100,0), '%')
      [26]: paste0(round(prop.table(flowchart_input %>% filter(ICU==F) %$% table(group))[1]*100,0), '%')
      [27]: paste0(round(prop.table(flowchart_input %>% filter(ICU==F) %$% table(group))[2]*100,0), '%')
      [28]: paste0(round(prop.table(flowchart_input %>% filter(ICU==F) %$% table(group))[4]*100,0), '%')
...
status.by.time.after.admission.plot <- function(aggregated.tbl, ...){
  plt <-  ggplot(aggregated.tbl)+ geom_bar(aes(x = day, fill = status), position = "fill", stat = 'count') +
